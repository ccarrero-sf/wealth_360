...
# Ensure the private key file is accessible by the Streamlit app.
# Use an absolute path or a path relative to where you run the Streamlit app.
PRIVATE_KEY_PATH = "/path/to/your/rsa_key.p8"
# PRIVATE_KEY_PASSPHRASE = os.environ.get("PRIVATE_KEY_PASSPHRASE") # Example using env var if key is protected
PRIVATE_KEY_PASSPHRASE = None # Set to your passphrase string if the key is encrypted
...
def create_snowpark_session():
    """Creates and returns a Snowpark session using key-pair authentication via file path."""
    add_log("INFO", "Attempting to create Snowpark session using private_key_file...")

    if SNOWFLAKE_ACCOUNT == "YOUR_SNOWFLAKE_ACCOUNT_IDENTIFIER" or SNOWFLAKE_USER == "YOUR_SNOWFLAKE_USER" or PRIVATE_KEY_PATH == "/path/to/your/rsa_key.p8":
         add_log("ERROR", "Cannot create session: Placeholder values for ACCOUNT, USER, or PRIVATE_KEY_PATH not replaced.")
         st.sidebar.error("Fatal Error: Snowflake connection details (Account, User, Private Key Path) must be set in the script.")
         return None

    if not os.path.exists(PRIVATE_KEY_PATH):
        add_log("ERROR", f"Private key file not found at {PRIVATE_KEY_PATH}")
        st.sidebar.error(f"Fatal Error: Private key file not found at {PRIVATE_KEY_PATH}. Please check the path.")
        return None
    ...
    connection_parameters = {
        ...
        **({"private_key_file_pwd": PRIVATE_KEY_PASSPHRASE} if PRIVATE_KEY_PASSPHRASE else {}),
    ...
    loggable_params = {k: v for k, v in connection_parameters.items() if k != 'private_key_file_pwd'}
    ...
    try:
        ...
        st.sidebar.error(f"Could not connect to Snowflake. Please check connection details, key file path, and passphrase (if set). Error: {e}")
        return None
